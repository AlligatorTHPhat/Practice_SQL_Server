...
-- 20. Cho biết danh sách trận đấu (NGAYTD, TENSAN, TENCLB1, TENCLB2, KETQUA) của câu lạc bộ có bxh thấp nhất trong bảng xếp hạng vòng 3 năm 2009.
...
WHERE NOT EXISTS (
    SELECT 1
    FROM CAULACBO CLB2
    WHERE CLB2.MACLB <> CLB.MACLB
    AND NOT EXISTS (
        SELECT 1 
        FROM TRANDAU TD
        WHERE (TD.MACLB1 = CLB.MACLB AND TD.MACLB2 = CLB2.MACLB AND TD.NAM = 2009)
           OR (TD.MACLB1 = CLB2.MACLB AND TD.MACLB2 = CLB.MACLB AND TD.NAM = 2009)
    )
);

-- 22. Cho biết mã câu lạc bộ, tên câu lạc bộ đã tham gia thi đấu với tất cả các câu lạc bộ còn lại (chỉ tính sân nhà) trong mùa giải 2009.
